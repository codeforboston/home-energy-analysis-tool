...
2. **`SubsectionTitle`** (new) — tier-2 heading. `text-2xl font-semibold text-zinc-950`. Props:
   - `as?: 'legend' | 'h3' | 'label'` (default `'h3'`) — render as `<legend>` when nested inside a `<fieldset>` (group heading, no single input owns it), or as `<label>` when the current markup is a real per-input label (e.g. "Living area", "Heating system efficiency %") so the label/input association isn't lost.
   - `htmlFor?: string` — required when `as="label"`; forwarded to the rendered `<label>`.
...
| File | Change |
|---|---|
| `HomeInformation.tsx` | "Home Information" legend → `SectionTitle`. "Address Information" and "Heating Design Temperature" legends → `SubsectionTitle as="legend"`. "Resident/Client Name(s)" — demote to tier-3 (`Field`'s default `Label`); it's a plain field, not a subsection. "Living Area (sf)" label → `SubsectionTitle as="label" htmlFor="living_area"` (keeps its `help`, keeps the input association). "Calculated Design Temperature" (read-only, no input) keeps a plain `Label`, just gets its `HelpButton` wrapped in `flex items-center gap-2`. Name and Design Temperature Override hand-rolled blocks → `Field` (Design Temperature Override uses `Field`'s new `help` + `description`). Remove local `titleClass`/`subtitleClass`/`descriptiveClass`/`componentMargin` constants — inline the literal Tailwind strings where no shared component replaces them. |
| `CurrentHeatingSystem.tsx` | "Existing Heating System" legend → `SectionTitle`. "Thermostat Settings" legend → `SubsectionTitle as="legend"`. "Heating System Efficiency %" label → `SubsectionTitle as="label" htmlFor="heating_system_efficiency_display"` with `help`. "Fuel Type" label — demote to tier-3 (plain `Label`, no subtitle styling); it was never listed as a subsection and doesn't group other fields. The 3 thermostat field labels (`Set point`, `Setback temperature`, `Setback hours per day`) currently use bare `font-bold` — **demote** to tier-3 (`Field`'s default `Label`), matching the Figma; they are not subsection titles. Hand-rolled field blocks (Set point, Setback temperature, Setback hours per day) → `Field` with `description`. Remove local constants as above. |
| `EnergyUseHistory.tsx` | "Energy Use History" legend → `SectionTitle`. Its `HelpButton` stays inline next to the title (removing it would be a content/behavior change, which is out of scope) but moves inside a `flex items-center gap-2` wrapper so it's vertically centered — same position, alignment bug fixed. No other heading exists in this file's current code — the spec's earlier draft referenced "Heating fuel usage" text that doesn't exist in the codebase; removed. |
...
| `CaseSummaryComponents/ErrorList.tsx` | Delete (duplicate of `forms.tsx`'s `ErrorList`); update its 4 callers (`HomeInformation.tsx`, `CurrentHeatingSystem.tsx`, `EnergyUseHistory.tsx`, and `routes/_heat+/Inputs1.tsx` — outside the case-edit page, but its import breaks once this file is deleted, so it gets the same one-line import swap) to import from `forms.tsx`. |
...
- Amended during plan-writing (2026-09-21): discovered promoting "Living area" and "Heating system efficiency %" to `SubsectionTitle` as originally worded would drop their real `<label htmlFor>` association (they are per-input labels, not group legends) — a behavior/a11y regression, not just a visual one. Gave `SubsectionTitle` an `as="label"` + `htmlFor` mode instead. Also corrected two inaccuracies found by re-reading the actual code against the migration table: "Fuel Type" and "Heating fuel usage" were never in the current code as subsection titles (the latter doesn't exist at all); demoted/removed those claims accordingly. Also corrected the `ErrorList.tsx` deletion row: it has 4 callers, not 1.
