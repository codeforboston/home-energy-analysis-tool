// Takes <?> or CSV input and <?>. Supports autosave/editing flow.
 ...
 */
export async function calculateWithCsv(formData: FormData, parsedForm: any) {
...
export async function calculateWithBills(
	...
	const overall_start_date = new Date(
		Math.min(...bills.map((b) => new Date(b.periodStartDate).getTime())),
	)
	const overall_end_date = new Date(
		Math.max(...bills.map((b) => new Date(b.periodEndDate).getTime())),
	)

	// Convert bills to Map<'records', ...> format expected by executeGetNormalizedOutput
	const billsPyReady: Map<
		'records',
		Array<{
	...
	> = new Map([
		[
			...
			bills.map((bill) => ({
				...
				inclusionOverride:
					typeof bill.inclusionOverride === 'boolean'
						? bill.inclusionOverride
							? 1
							: 0
						: bill.inclusionOverride || 0,
			})),
		],
	])
