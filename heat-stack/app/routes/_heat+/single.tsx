...
TableSchema, UserOverridesSchema} from '../../../types/index.ts'
...
const Schema = HomeFormSchema.and(CurrentHeatingSystemSchema).and(TableSchema).and(UserOverridesSchema) /* .and(HeatLoadAnalysisZod.pick({design_temperature: true})) */
...
export default function Inputs() {
    ...
    * "2020-10-02" 
    ...
    let dynamic_overrides: any = null

    let user_overrides_values = []
    ...
    if (show_usage_data && lastResult && hasDataProperty(lastResult)) {
        try {
            ...
            for (let record of usage_data.billing_records) {
                user_overrides_values.push(record.inclusion_override)
            }
            dynamic_overrides = user_overrides_values.reduce((acc, value, index) => {
                acc[`user_override_${index + 1}`] = value;
                return acc;
            }, {});
